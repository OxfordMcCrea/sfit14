...
os.chdir(r'/home/hartree2/mb/sann6100/Python_Sfit_Tests/Fourth_gen/Maxfit_Zoom')
...
# alleimage = np.flip(alleimage.swapaxes(1, 2), axis = 1)
# Instr_Func = np.flip(Instr_Func.swapaxes(1, 2), axis = 1)
...
if not os.path.exists('DataForMatt'):
    os.mkdir('DataForMatt')

print(alleimage.shape[0])
for x in range(alleimage.shape[0]):
    #print(x)
    np.savetxt(f'DataForMatt/{x}.dat', alleimage[x])
exit()

xrs, yrs = np.zeros((tot_img, tot_img)), np.zeros((tot_img, tot_img))
...
# plt.hist(theta.flatten()[top_side][np.less(energy.flatten()[top_side], 10)], bins = 90)
